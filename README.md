...
**v1.4 SDS upload:** **MarkItDown + regex** and **Hybrid** (MarkItDown â†’ OCR if no CAS) only â€” see [docs/SDS_EXTRACTION_PIPELINES.md](docs/SDS_EXTRACTION_PIPELINES.md). Optional **local LLM** (Ollama) for other flows: [docs/OLLAMA_SETUP.md](docs/OLLAMA_SETUP.md).
...
8. **SDS CAS extraction (two pipelines only)**
   ...
   - Benchmark: `python tests/test_extraction_pipelines.py --folder "../sds examples" --limit 20` â†’ `reports/extraction_benchmark.csv` and `extraction_benchmark_summary.md`.
...
â”‚   â”œâ”€â”€ run_sds_examples.py   # Batch run SDS extraction on PDFs in sds examples folder
