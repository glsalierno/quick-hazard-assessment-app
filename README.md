...
**v1.4 SDS upload:** **MarkItDown + regex** and **Hybrid** (MarkItDown to OCR if no CAS) only - see [docs/SDS_EXTRACTION_PIPELINES.md](docs/SDS_EXTRACTION_PIPELINES.md). Optional **local LLM** (Ollama) for other flows: [docs/OLLAMA_SETUP.md](docs/OLLAMA_SETUP.md).
...
8. **SDS CAS extraction (two pipelines only)**
 ...
 - Benchmark: `python tests/test_extraction_pipelines.py --folder "../sds examples" --limit 20` to `reports/extraction_benchmark.csv` and `extraction_benchmark_summary.md`.
