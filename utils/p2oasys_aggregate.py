...
``compute_p2oasys_scores`` stores each category's official-style aggregate in
``_category_max`` (mean of the two highest subcategory scores). This module
combines those per-category values into an overall score.
"""
...
def aggregate_category_scores(scores: dict[str, Any], method: str = "max") -> float:
    """
    Convert per-category scores (``_category_max``) to an overall evaluation.

    Methods:
      - max: overall = max of category-level scores
      - mean: overall = mean of category-level scores
