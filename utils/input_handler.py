...
def _extract_hazard_fields_from_pdf(pdf_bytes: bytes) -> dict[str, Any]:
    """
    Best-effort SDS hazard field extraction for P2OASys gap-fill.

    Converts PDF → markdown (MarkItDown when available), then regex-extracts
    GHS H-codes / flash point / vapor pressure / aquatic toxicity. Returns a
    flat dict suitable for ``p2oasys_source_bridges.sds_fields_to_extra_sources``.
    Failures return ``{}`` so CAS extraction is never blocked.
    """
    if not pdf_bytes:
        return {}
    try:
        from utils import sds_regex_extractor
        from utils.p2oasys_source_bridges import sds_parsed_result_to_extra_fields
    except Exception:
        return {}
    text = ""
    try:
        from parsers.markitdown_parser import MarkItDownParser
        from utils.cache_manager import CacheManager, pdf_fingerprint

        fp = pdf_fingerprint(pdf_bytes)
        parser = MarkItDownParser(cache=CacheManager())
        text = parser.convert_pdf_to_markdown(pdf_bytes, fp, force=False) or ""
    except Exception:
        text = ""
    if not text.strip():
        return {}
    try:
        parsed = sds_regex_extractor.extract_sds_fields_from_text(text)
        fields = sds_parsed_result_to_extra_fields(parsed if isinstance(parsed, dict) else {})
    except Exception:
        fields = {}
    # Deeper section mining (§9 state, §11 tox numbers, §12 eco, §5/8/10 cues).
    try:
        from v7.sds_structured import parse_structured_sds
        from utils.p2oasys_source_bridges import structured_sds_to_extra_fields

        parser = sds_regex_extractor.SDSParser()
        sections = {i: parser.extract_section_content(text, i) for i in range(1, 17)}
        structured = parse_structured_sds(sections, full_text=text)
        deep = structured_sds_to_extra_fields(structured)
        for k, v in (deep or {}).items():
            if v in (None, "", [], {}):
                continue
            if k not in fields or not fields.get(k):
                fields[k] = v
            elif k in ("eco_phrase_cues", "combustion_phrase_cues", "exposure_limits", "ghs_h_codes"):
                # OR-merge list pathway fills
                a = list(fields.get(k) or [])
...
class ChemicalInput:
    ...
    # Flattened SDS hazard fields for P2OASys gap-fill (H-codes, flash point, VP, aquatic).
    hazard_fields: dict[str, Any] = field(default_factory=dict)
...
class UnifiedInputHandler:
    ...
    def _process_sds_pdf_alternative(self, uploaded_file: Any, pdf_bytes: bytes, mode: str) -> ChemicalInput:
        ...
        return ChemicalInput(
            ...
            hazard_fields=_extract_hazard_fields_from_pdf(pdf_bytes),
        )
