...
checksum + PubChem checks, then ``_filter_sds_cas_pubchem_confirmed_only`` so the app only
lists CAS with ``pubchem_verified is True`` when ``USE_PUBCHEM_CAS_VALIDATION`` is on.
Gate knobs (``SHOW_ONLY_PUBCHEM_VERIFIED``, ``MIN_CAS_CONFIDENCE``) are read from ``config``
only — not ``sds_strategy`` session presets.
"""
...
# SDS list / auto-assess: only CAS confirmed in PubChem (``exists`` True).
...
def _filter_sds_cas_pubchem_confirmed_only(
    ...
    """
    SDS surface: keep only CAS with ``pubchem_verified is True``.
    If ``USE_PUBCHEM_CAS_VALIDATION`` is off, return inputs unchanged.
    If validation is on and there were scored CAS but none confirmed, return a user message.
    """
    import config as _cfg
