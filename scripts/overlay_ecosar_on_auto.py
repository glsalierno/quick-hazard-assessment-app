#!/usr/bin/env python3
"""
Overlay ECOSAR (EPI Suite API) aquatic predictions onto harvest auto P2OASys.

Gap-fills Ecological Acute Aquatic when missing; max-merges subcategory cells;
refreshes data/harvest_expert_vs_auto_density.json. Disable with HAZQUERY_SKIP_ECOSAR=1.
"""
...
import config
from utils import ecosar_client, hazard_for_p2oasys, p2oasys_scorer
...
ECO_ACUTE = (ECO_CAT, "Acute Aquatic Toxicity")
BATCH = 80
        ...
        rows.append(
            {
                ...
                "auto_only": auto_only,
                "gap_pp": round(100.0 * (expert_n - auto_n) / n, 1),
...
def snapshot(expert, auto, slots, auto6_slots, *, note: str):
    ...
    focus = {
        ...
        ("Environmental Fate & Transport", "Persistence"),
        ("Environmental Fate & Transport", "Rapid Degradability"),
        ("Environmental Fate & Transport", "Bioconcentration/ Bioaccumulation"),
    }
    thin = sorted(
...
def subcat_maxes_from_scores(scores: dict) -> dict[tuple[str, str], float]:
    ...
    for cat, bundle in (scores or {}).items():
        ...
        for sub, cell in bundle.items():
            ...
            if val is None:
                continue
            if cat_s == ECO_CAT and sub_s in ECO_SUBS:
                out[(ECO_CAT, sub_s)] = val
    return out


def main() -> None:
    if not ecosar_client.is_ecosar_enabled():
        raise SystemExit("ECOSAR disabled (HAZQUERY_SKIP_ECOSAR)")
    ...
    before = snapshot(expert, auto, slots, auto6_slots, note="before ECOSAR eco overlay")

    need = [cas for cas in sorted(expert) if ECO_ACUTE not in (auto.get(cas) or {})]
    print(f"expert={len(expert)} need_ecosar_acute={len(need)} api={ecosar_client.api_base_url()}")

    matrix = p2oasys_scorer.load_p2oasys_matrix(Path(config.P2OASYS_MATRIX_PATH))
    n_ok = n_fail = n_eco_cells = 0
    t0 = time.time()

    for i in range(0, len(need), BATCH):
        chunk = need[i : i + BATCH]
        batch = ecosar_client.submit_organics_batch([{"cas": c} for c in chunk], timeout=180.0)
        results = batch.get("results") or []
        # Map by index when CAS missing in payload
        for idx, cas in enumerate(chunk):
            pkg = results[idx] if idx < len(results) else None
            if not pkg or not pkg.get("ok"):
                # fallback single
                pkg = ecosar_client.submit_organics(cas=cas, use_cache=True)
            if not pkg or not pkg.get("ok"):
                n_fail += 1
                continue
            xs = ecosar_client.ecosar_to_extra_sources(pkg, existing_hazard={})
            if not xs:
                n_fail += 1
                continue
            n_ok += 1
            hd = hazard_for_p2oasys.build_hazard_data({}, extra_sources=xs)
            ...
            for key, sc in cells.items():
                if merge_score(amap, key, sc):
                    ...
                    amap[key] = float(sc)
        print(f"  … {min(i + BATCH, len(need))}/{len(need)} ok={n_ok} fail={n_fail} ({time.time() - t0:.0f}s)")
        time.sleep(0.4)
    ...
    after = snapshot(
        ...
        slots,
        auto6_slots,
        note=f"after ECOSAR eco; ok={n_ok}, fail={n_fail}, eco_cells={n_eco_cells}",
    )
    after["before"] = {
        k: before[k]
        ...
        try:
            for cas, amap in auto.items():
                ...
                for (cat, sub), sc in amap.items():
                    if cat == ECO_CAT and sub in ECO_SUBS:
