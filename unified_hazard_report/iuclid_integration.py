...
def render_reach_iuclid_panel(clean_cas: str) -> None:
    ...
    with st.expander("REACH / IUCLID (offline dossier)", expanded=False):
        ...
        quick_ctx = get_offline_context(fp) if fp else None
        cas_target_uuids = quick_ctx.uuids_for_cas(cas_norm) if quick_ctx is not None else []
        ...
        if eps_norm:
            ...
            st.caption("Each row is one normalized study record (not aggregated by template).")
            if wdf.empty:
