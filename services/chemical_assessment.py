...
RESULT_SCHEMA_VERSION = "v6_cas_1"
...
def _cached_cas_commonchem_detail(cas_rn: str) -> dict[str, Any]:
    """Cache normalized CAS Common Chemistry detail records for 24 hours."""
    client = cas_commonchem_client.CASCommonChemistryClient()
    return client.get_normalized_detail(cas_rn)
...
class AssessmentResult:
    ...
    cas_commonchem_data: Optional[dict[str, Any]] = None
    cas_commonchem_error: Optional[str] = None
...
class ChemicalAssessmentService:
    ...
    def __init__(self) -> None:
        ...
        self._dsstox_data: Optional[dict[str, Any]] = None
        self._refresh_db_capabilities()

    def _refresh_db_capabilities(self) -> None:
        """Re-read SQLite availability (CHEMICAL_DB_PATH may be set after worker start)."""
        ...
        if self.use_sqlite_dsstox:
            self._dsstox_data = None
        elif self._dsstox_data is None:
            self._dsstox_data = dsstox_local.load_dsstox_enhanced()

    @staticmethod
    def _resolve_preferred_name(
        identity: ChemicalIdentity,
        dsstox_info: Optional[dict[str, Any]],
        pubchem_data: Optional[dict[str, Any]],
        cas_commonchem_data: Optional[dict[str, Any]] = None,
    ) -> str:
        """
        Fallback order: DSSTox → CAS Common Chemistry → PubChem → typed input.
        """
        candidates = [
            (dsstox_info or {}).get("preferred_name"),
            (cas_commonchem_data or {}).get("name"),
            (pubchem_data or {}).get("title"),
            (pubchem_data or {}).get("iupac_name"),
            identity.chemical_name,
            identity.source_details if identity.source == InputSource.TYPED_NAME else None,
            identity.cas,
        ]
        for c in candidates:
    ...
    def to_result_data(self, result: AssessmentResult) -> dict[str, Any]:
        ...
        preferred = self._resolve_preferred_name(
            ident,
            result.dsstox_info,
            pubchem,
            result.cas_commonchem_data,
        )
        dtxsid = ident.dtxsid or (result.dsstox_info or {}).get("dtxsid")
        return {
            "result_schema_version": RESULT_SCHEMA_VERSION,
            "pubchem": pubchem,
            "cas_commonchem": result.cas_commonchem_data,
            "cas_commonchem_error": result.cas_commonchem_error,
    ...
    def _assess_single_identity(self, identity: ChemicalIdentity) -> AssessmentResult:
        ...
        try:
            ...
            if not pubchem_data and input_type == "cas":
                cid = pubchem_client.get_cid(cas, "cas")
                if cid:
                    pubchem_data = pubchem_client.get_compound_data(str(cid), input_type="cid")
        except Exception as e:
            fetch_error = f"PubChem fetch failed: {e}"
        if not pubchem_data and not fetch_error:
            fetch_error = (
                f"Compound not found in PubChem for CAS {cas}. "
                "Some CAS numbers resolve only via PubChem RN xref (not RegistryID)."
            )

        cas_commonchem_data = None
        cas_commonchem_error = None
        if cas_commonchem_client.get_api_key() and input_type == "cas":
            try:
                cas_commonchem_data = _cached_cas_commonchem_detail(cas)
            except cas_commonchem_client.CASCommonChemistryError as exc:
                # CAS is optional and must never block the hazard assessment.
                cas_commonchem_error = str(exc)
                ...
                toxval_data.setdefault(cat, []).append(
                    {
                        ...
                        "species": rec.get("species_common") or rec.get("species", ""),
                        "route": rec.get("exposure_route", ""),
                        "study_type": rec.get("study_type", ""),
                        "endpoint": rec.get("toxval_type") or rec.get("study_type", ""),
                        "toxval_type": rec.get("toxval_type", ""),
                    }
                )
        elif dtxsid:
        ...
        return AssessmentResult(
            ...
            cas_commonchem_data=cas_commonchem_data,
            cas_commonchem_error=cas_commonchem_error,
...
def get_assessment_service() -> ChemicalAssessmentService:
    """Return the shared GHhaz6 assessment service."""
